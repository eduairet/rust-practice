use file_system::{
    find_all_files_recursively, find_all_files_recursively_ignoring_case,
...
mod tests_directory_traversal {
    ...
    #[test]
    fn test_find_all_files_recursively_ignoring_case() {
        let path = "**/*.PNG";
        let files = find_all_files_recursively_ignoring_case(path).unwrap();
        println!("{:?}", files);
        assert!(files.contains(&"src/brisket.png".to_string()));
    }
}
